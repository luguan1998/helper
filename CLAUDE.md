...
客服助手 bot:在内部通讯软件(仅支持 `text` / `picture` 两种消息,**不支持 Markdown、不支持流式**)上,轮询 `link` CLI 拉消息 → 喂给 Claude → 把回复按规则发回。TypeScript + Node.js(ESM,Node ≥ 20)。

## 常用命令

```bash
npm install                 # 装依赖
npm run dev                 # 跑 bot(tsx 直跑 src/index.ts,需 link/claude/chromium 在环境可用)
...
npm run typecheck           # tsc --noEmit(检查 src + test)
npx vitest run test/pipeline.test.ts          # 跑单个测试文件
npx vitest run -t "图片消息"                   # 按用例名跑单个测试
```

## 关键约定

- **ESM + NodeNext**:所有相对导入必须带 `.js` 后缀(如 `import './llm.js'`),即便源文件是 `.ts`。新增文件务必遵守,否则运行时解析失败。
- **两份 tsconfig**:`tsconfig.json`(noEmit,类型检查 src+test,供 IDE/typecheck);`tsconfig.build.json`(只 emit `src/` 到扁平 `dist/`,排除 test)。`npm run build` 用后者。
- **类型先行**:端口接口(`Channel`/`Llm`/`Renderer`)与其具体适配器分文件放,接口用 `import type` 引用,避免测试拉起重的运行时依赖。

## 架构(深模块 + 3 接缝 + 配置驱动 pipeline)

核心是**一个深模块 `Assistant`**(`src/assistant.ts`):纯编排、无 I/O。主循环 = `轮询(channel) → 去重(seen) → runPipeline → outputPolicy → text/picture`;每条消息 try/catch,出错发纯文本致歉、循环不死。生产由后台循环驱动,测试用 `startLoop:false` 后直接 `await handle.tick()`。

**三个真接缝**(各有"生产适配器 + 测试假实现"两套,故为真接缝;其余依赖只有一个适配器、**不开端口**,勿投机性加 port):

| 接缝 | 端口 | 生产适配器 | 测试假 |
|---|---|---|---|
| 通讯渠道 | `src/channels/channel.ts` (`Channel`) | `link-channel.ts`(link CLI) | `test/fakes.ts` `FakeChannel` |
| 大模型 | `src/llm.ts` (`Llm`) | `claude-client.ts`(Claude CLI 子进程) | `FakeLlm` |
| 截图渲染 | `src/renderers/renderer.ts` (`Renderer`) | `puppeteer-renderer.ts`(Puppeteer) | `FakeRenderer` |
...
- 零配置 `runAssistant()` 即默认 vision+text 模型 + 接力 pipeline + markdownOutputPolicy。默认适配器在 `runAssistant` 内**懒加载**(动态 import),注入假时绝不加载 puppeteer/child_process——核心单测零外部依赖。

**按用户隔离会话**(`src/session-pool.ts`):`Map<userId, Session>` + LRU + `--resume` 续接;每用户 claudeSessionId 持久化于 `src/state.ts`(`~/.claude-bot/state.json`,可用 `BOT_STATE_DIR` 覆盖)。`SessionPool` 通过注入 `spawn` 工厂 + load/save 回调单测,无需真子进程。
...
- `link` CLI:`link get --number N` / `link send --user X --content "..."` / `link send --user X --picture <path>`。若 link 输出格式有变,**只改 `src/channels/link-channel.ts`** 一个适配器(接缝回报,不动架构)。
...
- 测试范式:核心逻辑注入 `test/fakes.ts` 的三假 + `startLoop:false` 驱动 `tick()`;`SessionPool` 注入假 spawn 工厂。生产适配器(link/puppeteer/claude)是外部工具薄包装,仅做轻量/集成验证。
